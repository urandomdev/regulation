...
type ItemQuery struct {
	...
	withSyncCursor *SyncCursorQuery
...
func (_q *ItemQuery) QueryAccounts() *AccountQuery {
...
// QuerySyncCursor chains the current query on the "sync_cursor" edge.
func (_q *ItemQuery) QuerySyncCursor() *SyncCursorQuery {
	query := (&SyncCursorClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withSyncCursor: _q.withSyncCursor.Clone(),
...
func (_q *ItemQuery) WithAccounts(opts ...func(*AccountQuery)) *ItemQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithSyncCursor tells the query-builder to eager-load the nodes that are connected to
// the "sync_cursor" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ItemQuery) WithSyncCursor(opts ...func(*SyncCursorQuery)) *ItemQuery {
	query := (&SyncCursorClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSyncCursor = query
	return _q
}
...
func (_q *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := _q.withAccounts; query != nil {
		if err := _q.loadAccounts(ctx, query, nodes,
			func(n *Item) { n.Edges.Accounts = []*Account{} },
			func(n *Item, e *Account) { n.Edges.Accounts = append(n.Edges.Accounts, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSyncCursor; query != nil {
		if err := _q.loadSyncCursor(ctx, query, nodes, nil,
			func(n *Item, e *SyncCursor) { n.Edges.SyncCursor = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ItemQuery) loadAccounts(ctx context.Context, query *AccountQuery, nodes []*Item, init func(*Item), assign func(*Item, *Account)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(account.FieldItemID)
	}
	query.Where(predicate.Account(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(item.AccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ItemQuery) loadSyncCursor(ctx context.Context, query *SyncCursorQuery, nodes []*Item, init func(*Item), assign func(*Item, *SyncCursor)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Item)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(synccursor.FieldItemID)
	}
	query.Where(predicate.SyncCursor(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(item.SyncCursorColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
